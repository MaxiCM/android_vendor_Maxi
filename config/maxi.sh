...
# Version 2.2, Adapted for MaxiCM.
...
if [ $ARCH = "64" ]; then
  # Get build version
  MAJOR=$(cat $DIR/vendor/maxi/vendor.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
  MINOR=$(cat $DIR/vendor/maxi/vendor.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
  MAINTENANCE=$(cat $DIR/vendor/maxi/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
  TAG=$(cat $DIR/vendor/maxi/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
  ...
  echo -e "${cya}Building ${bldcya}MaxiCM $VERSION for $DEVICE ${txtrst}";
  ...
  if [ -n "${INTERACTIVE}" ]; then
          ...
          if [ "${VENDOR}" == "maxi" ]; then
                  echo -e "[${bldgrn}lunch maxi_$DEVICE-userdebug${bldblu}]${txtrst}"
          else
